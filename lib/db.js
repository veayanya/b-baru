...
export async function getAllStoreData() {
 ...
 if (row.key === '__db_pool_state' || row.key === '__db_migration_history') continue;
 full[row.key] = row.data;
 }
 return full;
 });
}
...
export async function restoreAllStoreData(fullData) {
 ...
 if (key === '__db_pool_state') continue; // jangan timpa state pool
 const json = JSON.stringify(data);
 await sql`
 INSERT INTO app_store (key, data, updated_at)
 VALUES (${key}, ${json}, NOW())
...
export async function getActiveStorageInfo() {
 ...
 return { type: 'Neon PostgreSQL (Multi-DB Failover)', error: err.message };
 }
}
